...
from model.pw.model_2graph import MVCG, EarlyStopping, MyDataset, Predictor, TeeOutput


# parameters
device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
train_test_data_path = "../../dataset/processed/pw/processed_data_1.2.pth"
graph_data_path = "../../dataset/processed/pw/graphs_1.2.bin"
longtail_data_path = "../../dataset/processed/pw/longtail_data_threshold_10.npy"
logdir = "../../log/pw/hyperparameter/1.2"
# model parameters
embedding_dim = 64
...
def main():
    ...
    efficiency_df.to_excel(
        f'./train_analysis/graph_num/2/training_efficiency_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx', index=False)
    loss_df.to_excel(f'./train_analysis/graph_num/2/training_loss_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
                     index=False)
