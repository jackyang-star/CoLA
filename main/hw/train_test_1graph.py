...
def main():
    ...
    efficiency_df.to_excel(
        f'./train_analysis/graph_num/1/training_efficiency_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
        index=False)
    loss_df.to_excel(
        f'./train_analysis/graph_num/1/training_loss_{datetime.datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
        index=False)
